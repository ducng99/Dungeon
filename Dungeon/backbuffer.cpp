...
BackBuffer::Initialise(int width, int height)
{
	...
	{
		m_pWindow = SDL_CreateWindow("Dungeon Hunter", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, SDL_WINDOW_SHOWN);
	...
	TTF_Init();
	m_pFont = TTF_OpenFont("assets\\fonts\\DisposableDroidBB.ttf", 28);
	assert(m_pFont);
...
void
BackBuffer::DrawSprite(Sprite& sprite, float angle, bool flip)
{
	SDL_Rect src;
	...
	SDL_RenderCopyEx(m_pRenderer, sprite.GetTexture()->GetTexture(), &src, &dest, angle, 0, flip ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE);
}
...
Sprite* 
BackBuffer::CreateSprite(const char* pcFilename, float scale)
{
	assert(m_pTextureManager);
	...
	if (!pSprite->Initialise(*pTexture, scale))
	{
...
Sprite * BackBuffer::CreateSprite(const char * pcFilename, int x, int y, int width, int height, float scale)
{
	assert(m_pTextureManager);
	...
	Sprite* pSprite = new Sprite();
	if (!pSprite->Initialise(*pTexture, x, y, width, height, scale))
	{
		LogManager::GetInstance().Log("Sprite Failed to Create!");
	}
